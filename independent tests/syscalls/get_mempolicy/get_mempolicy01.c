...
int test_set_mempolicy_default(struct test_case *tc);

int test_set_mempolicy_none(struct test_case *tc);

int test_mbind_none(struct test_case *tc);

int test_mbind_default(struct test_case *tc);
...
int test_set_mempolicy_default(struct test_case *tc)
{
set_mempolicy(tc->policy, nodemask->maskp, nodemask->size);
	return TST_RET;
}

int test_set_mempolicy_none(struct test_case *tc)
{
set_mempolicy(tc->policy, NULL, 0);
	return TST_RET;
}

int test_mbind(struct test_case *tc, unsigned long *maskp, unsigned long size)
{
	tc->addr = mmap(NULL, MEM_LENGTH, PROT_READ | PROT_WRITE,
			     MAP_PRIVATE | MAP_ANONYMOUS, 0, 0);
mbind(tc->addr, MEM_LENGTH, tc->policy, maskp, size, 0);
	return TST_RET;
}
