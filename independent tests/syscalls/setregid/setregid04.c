...
gid_t first_gid, second_gid, root_gid, neg_one = -1;
struct test_data_t {
	gid_t *real_gid;
	gid_t *eff_gid;
	gid_t *exp_real_usr;
...
int  gid_verify(gid_t rg, gid_t eg, const char *when)
{
...
{
SETREGID(*test_data[i].real_gid, *test_data[i].eff_gid);
	if (TST_RET == -1) {
		tst_res(TFAIL | TTERRNO, "setregid(%d, %d) failed",
			*test_data[i].real_gid, *test_data[i].eff_gid);
		return;
	}
int 	gid_verify(*test_data[i].exp_real_usr, *test_data[i].exp_eff_usr,
