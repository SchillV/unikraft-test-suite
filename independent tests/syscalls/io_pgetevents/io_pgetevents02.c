...
io_context_t ctx, invalid_ctx = 0;

int fd, ctx_initialized;
...
void setup(void)
{
	...
	cbs[0] = &cb;
	sigemptyset(&sigmask);
	fd = open("io_pgetevents_file", O_RDWR | O_CREAT, 0644);
...
tv->io_pgetevents(*tc->ctx, tc->min_nr, tc->max_nr, tc->events, t;
			       sigmask));
	if (TST_RET == 1) {
