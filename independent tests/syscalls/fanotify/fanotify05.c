...
void event_res(struct fanotify_event_metadata *event, int i)
{
	int len = 0;
	const char *filename;
...
void test_fanotify(unsigned int n)
{
	...
	fd_notify = fanotify_init(tc->init_flags | FAN_NONBLOCK, O_RDONLY);
	fanotify_mark(fd_notify, FAN_MARK_MOUNT | FAN_MARK_ADD, FAN_OPEN,
			   AT_FDCWD, MOUNT_PATH);
...
void setup(void)
{
	...
	if (access(SYSFS_MAX_EVENTS, F_OK) && errno == ENOENT)
		...
		file_scanf(SYSFS_MAX_EVENTS, "%d", &max_events);
