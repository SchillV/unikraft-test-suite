...
char fname1[BUF_SIZE + 11], fname2[BUF_SIZE + 11];

char dname1[BUF_SIZE], dname2[BUF_SIZE], tmpdir[BUF_SIZE];
...
void do_test(unsigned int number)
{
	 ...
	 */
	fanotify_mark(fd_notify, FAN_MARK_ADD | mark->flag, tc->mask,
			   AT_FDCWD, MOUNT_PATH);
	 ...
	 */
	mkdir(dname1, 0755);
	mount(dname1, dname1, "none", MS_BIND, NULL);
	fanotify_save_fid(dname1, &dir_fid);
	if (report_target_fid)
		subdir_fid = &dir_fid;
	if (tc->sub_mask)
		fanotify_mark(fd_notify, FAN_MARK_ADD | sub_mark->flag,
	...
	dirfd = open(dname1, O_RDONLY | O_DIRECTORY);
	close(dirfd);
	umount(dname1);
	...
	if (!fs_mark || rename_ignored) {
		rename(dname1, tmpdir);
		rename(tmpdir, dname2);
	} else {
		rename(dname1, dname2);
	}
	rmdir(dname2);
	len += read(0, fd_notify, event_buf + len, EVENT_BUF_LEN - len);
	 * FAN_RENAME event is independent of MOVED_FROM/MOVED_TO and not merged
