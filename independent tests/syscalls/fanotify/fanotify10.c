...
int fd_notify[NUM_CLASSES][GROUPS_PER_PRIO];

int fd_syncfs;

char event_buf[EVENT_BUF_LEN];

int event_buf_pos, event_buf_len;
...
int evictable_mark_unsupported;
...
int bind_mount_created;

unsigned int num_classes = NUM_CLASSES;
...
struct fanotify_mark_type fanotify_mark_types[] = {
...
int format_path_check(char *buf, const char *fmt, int count, int i)
{
	int limit = count ? : 1;
	if (i >= limit)
		return 0;
...
void show_fanotify_ignore_marks(int fd, int min, int max)
{
...
int create_fanotify_groups(unsigned int n)
{
	...
	fd_syncfs = open(MOUNT_PATH, O_RDONLY);
...
int 			 * When using FAN_MARK_IGNORE, verify that the FAN_EVENT_ON_CHILD
...
void mount_cycle(void)
{
	...
	mount(MOUNT_PATH, MNT2_PATH, "none", MS_BIND, NULL);
	bind_mount_created = 1;
}

int int verify_event(int p, int group, struct fanotify_event_metadata *event,
...
int generate_event(struct tcase *tc, unsigned long long expected_mask)
{
	int fd, status;
	child_pid = fork();
	if (child_pid == 0) {
	...
	for (p = 0; p < num_classes; p++) {
		...
		for (i = 0; i < GROUPS_PER_PRIO; i++) {
			...
			while ((event = fetch_event(fd_notify[p][i], &ret))) {
				event_count++;
int 				if (!verify_event(p, i, event, p == 0 ?
						tc->expected_mask_without_ignore :
...
void setup(void)
{
	...
	cp(TEST_APP, FILE2_EXEC_PATH);
	mkdir(MNT2_PATH, 0755);
	mount_cycle();
