...
void run(unsigned int test_case)
{
	if (fork()) {
		tst_reap_children();
		return;
	}
unshare(CLONE_NEWNS | CLONE_FS);
	if (TST_RET == -1)
	...
	mount("none", CHROOT_DIR, "tmpfs", 0, 0);
	chroot(CHROOT_DIR);
	mkdir(NEW_ROOT, 0777);
	...
	if (test_cases[test_case].test_case == PUT_OLD_NOT_UNDERNEATH_NEW_ROOT) {
		...
		mkdir(PUT_OLD_FS, 0777);
		mount("none", PUT_OLD_FS, "tmpfs", 0, 0);
		mkdir(PUT_OLD_BAD, 0777);
	} else {
...
syscall(__NR_pivot_root, NEW_ROOT, actual_put_old);
